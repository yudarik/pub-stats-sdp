...
function fetchData(query, urlConfig){
    var dfd = q.defer();
...
module.exports = function (query, urlConfig, done) {
    ...
    if(query.dimension.indexOf("combined")>=0){
        ...
        getDataPromise = q.all(_.map([
            ...
            'deviceType',
            'connectionType'
        ], dim => fetchData(_.extend({},query,{dimension:[dim]}), urlConfig).then(data=>({dim,data}))))
        .then(result=>{
            var data={};
            ...
            //============================================================
            //TODO: delete and add appropriate dimension above once adType will work on sdp!!!!!
            data.adType=[];
            //============================================================
        ...
        getDataPromise = fetchData(query, urlConfig);
    }
